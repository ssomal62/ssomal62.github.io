---
layout : single
title : "[JAVA] Date와 Calendar를 사용하면 안되는 이유" 
categories : Java
tag : [Java]
---
...
수업 시간에 Date를 사용할 때마다 나타나는 취소선이 너무도 거슬렸다.  (eclipse 별도 테마를 설치하고 난 후로는 취소선이 없어졌다.) 그 취소선 때문에 Date는 사용하기 싫었고 Calendar를 제대로 사용하기 위해 검색을 했다. 그런데  전문가들의 블로그에서  Date뿐만 아니라 Calendar도 사용을 하지 말라는 얘기가 너무나 많았다. 왜 그럴까? 그럼 어떤 API를 사용하는게 옳은 것일까? 
...
`get`으로 불러와 수정하는 것이 아니라 `set`으로 수정할 수 있다는 점에서 벌써 쓰지 말아야 할 이유가 생긴 것 같은데 그 외에도 여러가지 문제점들이 이 두 클래스들을 사용하지 말아야 할 이유를 못박는다.
...
![웹 캡처_12-9-2023_184638_docs.oracle.com](assets/images/웹 캡처_12-9-2023_184638_docs.oracle.com.jpeg)
...
//10월로 구하려고했는데 나오는 값은 9월
    
```

이 외에도 더 많은 이유가 있는데 나중에 여유가 되면 추가적으로 정리를 해봐야겠다.

그렇다면 그러면 어떤 것을 사용해야할까?
...
얼핏 보기에 가독성이 문제가 별로 없어보여 찾아보았는데 포맷 지원 시스템에서 차이가 나는 듯 했다. Date는 SimpleDateFormat을 잉요해서 포맷하고, java.time은 DateTimeFormatter 를 사용한다. 
...
Date와 Calendar 사용을 지양해야한다고는 하나 기업에서 이미 Date나 Calendar를 사용 중인 경우가 많다고하니.. 애석하게도 결국은 모두 알아두어야 하는셈이다. 별도의 프로젝트일 경우에는 가능하면 java.time API를 쓰려고 노력은 해야겠다.



<참고한 사이트>

[Java의 날짜/시간 API. Date와 Calendar는 왜 사용하면 안될까? — 기록으로 채워가는 개발자 이야기 (tistory.com)](https://gaebalsogi.tistory.com/66#Calendar%2C Date 클래스의 문제점-1)

[[Java\] Java의 날짜, 시간 클래스는 어떻게 편해졌나 (tistory.com)](https://marrrang.tistory.com/48)

[Java의 날짜와 시간 API (naver.com)](https://d2.naver.com/helloworld/645609)
