...
항상 팀원의 성장을 바라시는 조조장님께서 추석 과제로 자료구제 문제를 내주셨다. 찾아보며 푸는 일명 노베이스에 욱여넣기.
...
* <span style="font-size: smaller"> (예시 코드를 넣기엔 너무 길어서, 그냥 인터럽트 스택 방식으로 처리한다 라고만 알고있자.)</span>
...
#### 퀵 정렬
  ...
  * 원리
    * 피벗값 : 임의의 값.
    * 피벗값을 중심으로 피벗보다 큰 값들의 배열, 피벗보다 작은 값들의 배열로 나눈다. (이 작업을 partition step이라고 함)
    * 각 배열에서 새로운 피벗을 잡고 다시 정렬.
    * 이 과정을 각각의 크기가 0이나 1이 될 때까지 진행
...
#### 버블 정렬
  ...
  * (수업 시간에 많이 했으니 원리는 패쓰)


#### 쉘 정렬
  * 삽입 정렬의 성질을 이용, 보완한 삽입정렬의 일반화.
